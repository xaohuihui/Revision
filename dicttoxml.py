...
# 最大深度，若超过这个深度的内容 都设置为字符串  写入一个标签中
max_level = float('inf')
...
def convert(obj, ids, attr_type, item_func, cdata, parent='root', level=1):
    ...
    if isinstance(obj, numbers.Number) or type(obj) in (str, unicode):
        return convert_kv(item_name, obj, attr_type, cdata, level=level)

    if hasattr(obj, 'isoformat'):
        return convert_kv(item_name, obj.isoformat(), attr_type, cdata, level=level)

    if type(obj) == bool:
        return convert_bool(item_name, obj, attr_type, cdata, level=level)

    if obj is None:
        return convert_none(item_name, '', attr_type, cdata, level=level)

    if isinstance(obj, dict):
        return convert_dict(obj, ids, parent, attr_type, item_func, cdata, level=level)

    if isinstance(obj, iterable):
        return convert_list(obj, ids, parent, attr_type, item_func, cdata, level=level)

    raise TypeError('Unsupported data type: %s (%s)' % (obj, type(obj).__name__))


def convert_dict(obj, ids, parent, attr_type, item_func, cdata, level=0):
    ...
    for key, val in obj.items():
        ...
        if level >= max_level and isinstance(val, dict):
            val = str(val)
        ...
        if isinstance(val, numbers.Number) or type(val) in (str, unicode):
            addline(convert_kv(key, val, attr_type, attr, cdata, level=level))

        elif hasattr(val, 'isoformat'):  # datetime
            addline(convert_kv(key, val.isoformat(), attr_type, attr, cdata, level=level))

        elif type(val) == bool:
            addline(convert_bool(key, val, attr_type, attr, cdata, level=level))
            ...
            addline('\n' + '\t' * level + '<%s%s>%s</%s>' % (
                ...
                convert_dict(val, ids, key, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                key
            )
                    )
            ...
            addline('\n' + '\t' * level + '<%s%s>%s</%s>' % (
                ...
                convert_list(val, ids, key, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                key
            )
                    )

        elif val is None:
            addline(convert_none(key, val, attr_type, attr, cdata, level=level))
...
def convert_list(items, ids, parent, attr_type, item_func, cdata, level=0):
    ...
    for i, item in enumerate(items):
        ...
        if level >= max_level and isinstance(item, dict):
            item = str(item)
        ...
        if isinstance(item, numbers.Number) or type(item) in (str, unicode):
            addline(convert_kv(item_name, item, attr_type, attr, cdata, level=level))

        elif hasattr(item, 'isoformat'):  # datetime
            addline(convert_kv(item_name, item.isoformat(), attr_type, attr, cdata, level=level))

        elif type(item) == bool:
            addline(convert_bool(item_name, item, attr_type, attr, cdata, level=level))

        elif isinstance(item, dict):
            if not attr_type:
                addline('\n' + '\t' * level + '<%s>%s</%s>' % (
                    item_name,
                    convert_dict(item, ids, parent, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                    item_name,
                )
                        )
            else:
                addline('\n' + '\t' * level + '<%s type="dict">%s</%s>' % (
                    item_name,
                    convert_dict(item, ids, parent, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                    item_name,
                )
            ...
            if not attr_type:
                addline('\n' + '\t' * level + '<%s %s>%s</%s>' % (
                    ...
                    convert_list(item, ids, item_name, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                    item_name,
                )
                        )
            else:
                addline('\n' + '\t' * level + '<%s type="list"%s>%s</%s>' % (
                    item_name, make_attrstring(attr),
                    convert_list(item, ids, item_name, attr_type, item_func, cdata, level=levelN) + '\n' + '\t' * level,
                    item_name,
                )
            ...
            addline(convert_none(item_name, None, attr_type, attr, cdata, level=level))
...
def convert_kv(key, val, attr_type, attr={}, cdata=False, level=0):
...
def convert_bool(key, val, attr_type, attr={}, cdata=False, level=0):
    ...
    if attr_type:
        attr['type'] = get_xml_type(val)
    attrstring = make_attrstring(attr)
    return '\n' + '\t' * level + '<%s%s>%s</%s>' % (key, attrstring, unicode(val).lower(), key)


def convert_none(key, val, attr_type, attr={}, cdata=False, level=0):
    ...
    if attr_type:
        attr['type'] = get_xml_type(val)
    attrstring = make_attrstring(attr)
    return '\n' + '\t' * level + '<%s%s></%s>' % (key, attrstring, key)


def dicttoxml(obj, root=True, custom_root='root', ids=False, attr_type=True,
              item_func=default_item_func, cdata=False, max_lev=max_level):
